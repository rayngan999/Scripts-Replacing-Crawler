...
class TaskManager:
    ...
    def __init__(self, manager_params, browser_params,portNumber, process_watchdog=False):
        ...
        self.browsers = self._initialize_browsers(browser_params, portNumber)
    ...
    def _initialize_browsers(self, browser_params, portNumber):
        """ initialize the browser classes, each its unique set of params """
        browsers = list()
        for i in range(self.num_browsers):
            browser_params[i][
                'crawl_id'] = self.data_aggregator.get_next_crawl_id()
            browsers.append(Browser(self.manager_params, browser_params[i], portNumber))
